...
#include "fstream"
#include "TLegend.h"
void LangausFitRange(const char* fileName, const char* histoPath, double totmin, double totmax )
{
  ...
  TH1F* clusterSignal_d20 = (TH1F*)  gDirectory->Get(histoPath);
  if ( clusterSignal_d20 == 0x0 ) {
    std::cout << histoPath << " histo was not found/open.\nExiting...\n";
    exit( -2 );
  }
...
// determining tot range
  ...
  if ( xmax < 0 ) { 
    xmax = clusterSignal_d20->GetBinLowEdge(nbins+1);
  }
  ...
  tot.setRange("Range",totmin,totmax);
  char fit_range[256];
  ...
  RooRealVar landau_mean("landau_mean","mean landau",mean*0.8,totmin,totmax) ;
  RooRealVar landau_width("landau_width","sigma landau",1,0.1,5) ;
  RooLandau landau("lx","lx",tot,landau_mean,landau_width) ;

  // Construct gauss(t,mg,gaussian_sigma)
  RooRealVar mg("mg","mg",0) ;
  RooRealVar gaussian_sigma("gaussian_sigma","gaussian_sigma",0.5,0.1,5) ;
